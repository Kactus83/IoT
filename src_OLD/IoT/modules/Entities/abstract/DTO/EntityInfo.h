...
struct EntityInfo {
    ...
    EntityInfo(const DeviceConfig& config, const EntitySettings& settings)
    {
        ...
        this->name = strdup(settings.name);
        ...
        snprintf(buffer, sizeof(buffer), "%s/%s/get", config.INTEGRATION_NAME, this->unique_id);
        this->getTopic = strdup(buffer);

        snprintf(buffer, sizeof(buffer), "%s/%s/set", config.INTEGRATION_NAME, this->unique_id);
